...
export async function obtenerDatosPedidos() {
  try {
    const response = await fetch('http://192.168.78.98:5150/obtener-pedidos');
    if (!response.ok) {
...
export const actualizarStockG = async () => {
  try {
    const response = await axios.put('http://192.168.78.98:5150/actualizar-stockG');
...
export const actualizarStockSemana = async (numero, id_dia, nuevoStock) => {
  try {
    const response = await axios.put(
      `http://192.168.78.98:5150/actualizar-stock/${numero}/${id_dia}`,
      { stockD: nuevoStock }
    );
    return response.data;
  } catch (error) {
...
export const actualizarSemana = async (numero, id_dia, id_menu) => {
  try {
    const response = await axios.put(
      `http://192.168.78.98:5150/actualizar-semana/${numero}/${id_dia}`,
      { id_menu }
    );
    return response.data;
  } catch (error) {
...
export async function obtenerDatosMenu() {
  try {
    const response = await fetch('http://192.168.78.98:5150/datosMenu');
    if (!response.ok) {
...
export async function obtenerDatosSemana() {
  try {
    const response = await fetch('http://192.168.78.98:5150/datosSemana');
    if (!response.ok) {
...
export async function obtenerPrecios() {
  try {
    const response = await fetch('http://192.168.78.98:5150/precio_colaciones');
    if (!response.ok) {
...
export async function DataVentas() {
  try {
    const response = await fetch('http://192.168.78.98:5150/Ventas');
    if (!response.ok) {
      throw new Error('Error al obtener datos de precio_colaciones');
    }
    return await response.json();
...
export const insertarVenta = async (ventaData) => {
  try {
    const response = await axios.post(
      'http://192.168.78.98:5150/insertar-venta',
      ventaData
    );
    return response.data;
  } catch (error) {
...
export const insertarPedido = async (datos) => {
  ...
  try {
    const response = await axios.post('http://192.168.78.98:5150/insertar-pedido', datos_ordenados);
    return response.data;
  } catch (error) {
    console.error('Error al enviar la solicitud POST:', error);
    throw error;
...
export const actualizarEstado = async (barra) => {
  try {
    const response = await axios.put(
      `http://192.168.78.98:5150/actualizar-estado/${barra}`,

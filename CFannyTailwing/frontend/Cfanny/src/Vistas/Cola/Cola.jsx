...
import { faBreadSlice, faIceCream, faWineBottle, faFile, faReceipt } from '@fortawesome/free-solid-svg-icons';
...
export default function Cola() {
  ...
  function ConfirmarVenta(n) {
    handleDelete(n.barra);
    alert("VENTA ACEPTADA"); // Mostrar alerta del navegador
  }
  ...
  <tr>
    <th className="px-2 py-3 text-center text-2xl font-medium text-black-500 uppercase tracking-wider">
      N
    </th>
    <th className="px-4 py-3 text-center text-2xl font-medium text-black-500 uppercase tracking-wider">
      Texto Orden
    </th>
    <th className="px-2 py-3 text-center text-2xl font-medium text-black-500 uppercase tracking-wider">
      Cantidad
    ...
    <th className="px-2 py-3 text-center text-2xl font-medium text-black-500 uppercase tracking-wider w-32">
      Acciones
    </th>
    <th className="px-2 py-3 text-center text-2xl font-medium text-black-500 uppercase tracking-wider w-32">
      Cancelar
...
<tbody className="bg-white divide-y divide-black-200">
  {mergedPedidos.map((pedido, index) => (
    ...
    >
      {pedido.rowSpan > 0 && (
        <td className="px-2 py-4 whitespace-nowrap text-xl text-center" rowSpan={pedido.rowSpan}>
          {pedido.numeroOrden}
        </td>
      )}
      <td className="px-4 py-4 whitespace-nowrap text-xl text-center">{pedido.textoOrdenConIcono}</td>
      <td className="px-2 py-4 whitespace-nowrap text-xl text-center">{pedido.cantidad}</td>
      <td className="px-2 py-4 whitespace-nowrap text-xl text-center">{pedido.comentario}</td>
      <td className="px-2 py-4 whitespace-nowrap text-xl text-center">$ {parseInt(pedido.precio, 10)}</td>
      <td className="px-2 py-4 whitespace-nowrap text-xl text-center w-32">
        <button
          onClick={() => {
            setSelectedPedido(pedido);
            setShowModal(true);
        ...
        <button
          onClick={() => activar_empanada(pedido)}

...
const MenuTable = ({ datosMenu, handleEliminarItem }) => {
  ...
  useEffect(() => {
    ...
    obtenerDatosTiposMenu()
      ...
      .catch((error) =>
        console.error("Error al obtener datos de tipos de menú:", error)
      );
  }, []);
  ...
  useEffect(() => {
    ...
    setDatosFiltrados(
      datosMenu.filter((item) => {
        const tipoValido =
          filtroTipo === "" || item.tipo === parseInt(filtroTipo, 10);
        const nombreValido =
          filtroNombre === "" ||
          item.nombre.toLowerCase().includes(filtroNombre.toLowerCase());
        const precioValido =
          (filtroPrecioMin === "" ||
            item.precio >= parseFloat(filtroPrecioMin)) &&
          (filtroPrecioMax === "" ||
            item.precio <= parseFloat(filtroPrecioMax));

        return (
          tipoValido &&
          nombreValido &&
          precioValido &&
          tiposValidos.includes(item.tipo)
        );
      })
    );
                      ...
                      <button
                        onClick={() => {
                          if (
                            ...
                            handleEliminarItem(item.nombre);
                          }
                        }}

...
export default function NuevoItem({ onInsertSuccess }) {
  ...
  const handleInsertarMenu = async () => {
    try {
      ...
      onInsertSuccess(); // Call the callback prop
    } catch (error) {
        ...
        <div className="w-1/2">
          <div className="grid grid-cols-2 gap-4">
            {opcionesTipo.map(opcion => (
              <button
                key={opcion.id_tipo}
                className={`px-4 py-2 border border-gray-300 rounded text-black ${tipoNuevo === opcion.id ? 'bg-blue-500 text-white' : 'bg-white'}`}
